...
logging.basicConfig(
    filename=dt_string + ".log", encoding="utf-8", level=logging.DEBUG, format="%(asctime)s %(levelname)s %(message)s"
)
...
ICON_CANCEL = (
    b"iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAACXBIWXMAAA7EAAAOxAGVKw4bAAACCklEQVRYhb2Xz0obURTGfwkhS"
...
ICON_WARNING = (
    b"iVBORw0KGgoAAAANSUhEUgAAACAAAAAgCAYAAABzenr0AAAACXBIWXMAAA7EAAAOxAGVKw4bAAACDUlEQVRYhcXXz0sVURQH8M8M"
...
def collapse(layout, key, visible):
    ...
    The purpose of this function is to create a collapsible section in a GUI using the PySimpleGUI library. By default,
...
def create_main_window():
    ...
    hidden_sec = [
        ...
        [
            sg.InputText(
                ...
                tooltip="Enter the language which should be scraped.",
                key="inputtxt",
                border_width=0,
            )
        ],
    ...
    layout = [
        [sg.Text("Start number for each language:", border_width=0)],
        [
            sg.InputText(
                default_text=1,
                text_color="lightgray",
                tooltip="Enter the start number (int) which should be used for scraping.",
                border_width=0,
            )
        ],
        ...
        [
            sg.InputText(
                default_text=2,
                text_color="lightgray",
                tooltip="Enter the end number (int) which should be used for scraping.",
                border_width=0,
            )
        ],
        ...
        [
            sg.InputText(
                default_text=1,
                text_color="lightgray",
                tooltip="Enter in what steps numbers should be scraped between Start-End",
                border_width=0,
            )
        ],
        ...
        [
            sg.InputText(
                default_text=CURRENT_WORKING_DIR,
                text_color="lightgray",
                tooltip="Enter desired path for the csv-file!",
                border_width=0,
            ),
            sg.FolderBrowse(),
        ],
    ...
    while True:
        ...
        if event == "OK":
            try:
                ...
                logging.error("Values 0-2 contains non integer values.: %s", valerr)
...
def create_pop_up_window(mode=""):
    """
    Creates a pop-up window in case the user has entered non integer values in the text fields for the number
    intervalls and steps or an invalid path for saving the csv file.
    :return: None
    """
    if mode == "path":
...
if __name__ == "__main__":
    ...
    while retries < MAX_RETRIES_REQ:
        try:
            ...
            page = requests.get(
                "https://www.languagesandnumbers.com/site-map/en/", headers=HEADERS, timeout=REQ_TIMEOUT
            )
    ...
    sg.SystemTray.notify(
        "Started Script", "Scraping languagesandnumbers.com...", display_duration_in_ms=750, fade_in_duration=0
    )
    ...
    for link in lst_of_links:
        ...
        for i in range(start, end, step):
            ...
            if lang == "pld" and i > 12:
                logging.warning(
                    "POSTING HTTP to ajax server is not supported for %s with values larger than 12.", str(lang)
                )
                continue
            ...
            with requests.Session() as session:
                while retries < MAX_RETRIES_REQ:
                    try:
                        ...
                        logging.info(
                            "POST method - params: {numberz: %s, lang: %s (%s)}",
                            str(i),
                            str(lang),
                            str(find_lang_long(link)),
                        )
                ...
                if soup.get_text().split(":", 3)[-1].split(".")[0] == "This number is too big":
                    logging.warning(
                        "Not supported. This number is too big. (Lang: %s (%s))", str(lang), str(find_lang_long(link))
                    )
